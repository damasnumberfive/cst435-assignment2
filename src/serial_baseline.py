...
import csv  # For saving baseline value
...
def main():
    ...
    # This is to determine the project path, ensure script run correctly
    ...
    global_start_time = time.time()  # Global start time. This is for the total execution of benchmark
    ...
    # Serial Execution Summarization
    print(f"{'='*60}")
    ...
    # Save Serial Baseline Value to CSV
    # Save the total execution time as the baseline for speedup calculations
    baseline_csv_path = os.path.join(project_root, "serial_baseline_value.csv")
    try:
        with open(baseline_csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['metric', 'value'])
            writer.writerow(['serial_baseline', total_time])
            writer.writerow(['num_images', len(image_paths)])
            writer.writerow(['timestamp', time.strftime('%Y-%m-%d %H:%M:%S')])
        
        print(f"\n{'='*60}")
        print(f"✅ Serial Baseline (T₁): {total_time:.4f}s")
        print(f"✅ Saved to: {baseline_csv_path}")
        print(f"{'='*60}")
        print(f"\nYou can now run:")
        print(f"  - python3 src/multiprocessing_image.py")
        print(f"  - python3 src/concurrent_futures.py")
        print(f"\nThey will automatically use this baseline for speedup calculations.")
        print(f"{'='*60}\n")
        
    except Exception as e:
        print(f"❌ Error: Could not save serial baseline value. Error: {e}")
